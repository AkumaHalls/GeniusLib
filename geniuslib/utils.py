...
def clean_asset_name(name: str) -> str:
    ...
    Regras do ClashKingAssets:
    - lower case
    - espaços -> _
    - remove pontos, ?, aspas simples

    Parameters
    ----------
    name: :class:`str`
        O nome original (ex: ``"Barbarian King"``, ``"Santa's Surprise"``)

    Returns
    -------
    :class:`str`
        O nome limpo (ex: ``"barbarian_king"``, ``"santa_s_surprise"``)
    """
    return (
        name.lower()
        .replace(" ", "_")
...
def asset_path(category: str, name: str, level: int = None, subcategory: str = None) -> str:
    """Gera o caminho relativo do asset para a estrutura de diretórios local.

    Parameters
    ----------
    category: :class:`str`
        Categoria do asset (ex: ``"troops"``, ``"heroes"``, ``"spells"``, ``"equipment"``,
        ``"buildings/home-village"``, ``"pets"``, ``"leagues/league-tier"``)
    name: :class:`str`
        Nome da unidade/item (ex: ``"Barbarian"``, ``"Lightning Spell"``)
    level: :class:`int`, optional
        Nível do asset (usado para buildings, traps)
    subcategory: :class:`str`, optional
        Subcategoria (ex: ``"home-village"``, ``"builder-base"``)
